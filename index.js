...
client.on('message', async msg => {
    ...
    const cmd = texto.toLowerCase();
     ...
     if (cmd.startsWith("/setstock")) {
        ...
        if (isNaN(quantidade)) return msg.reply("❌ Uso: /setstock QUANTIDADE [DATA HORA]\nExemplo: /setstock 1000 04/07/2025 11:59");
        ...
        if (partes.length >= 4) {
            ...
            if (!/^\d{2}\/\d{2}\/\d{4} \d{2}:\d{2}$/.test(dataHora)) {
                return msg.reply("❌ Data/hora inválida. Use o formato: DD/MM/AAAA HH:MM");
            }
    
            info.proximoEstoque = dataHora;
            ...
            if (dataEstoque > agora) {
                msg.reply(`📦 Estoque agendado!\n🗓️ Chegará: *${dataHora}*\n💠 Quantidade: *${quantidade} Robux*`);
            } else {
                msg.reply(`✅ Estoque atualizado para *${quantidade} Robux* disponível imediatamente.`);
            }
        } else {
    ...
    🎯 Calcular Game Pass: */gamepass QUANTIDADE*`;
        ...
        if (texto.trim().toLowerCase() !== "/valor") {
            return msg.reply("❌ Comando inválido. Use apenas */valor* (sem nada depois) para iniciar a conversão.");
        }
    
        aguardandoValor[msg.from] = { etapa: "tipo" };
        msg.reply(
    ...
    // E dentro do mesmo on('message'), abaixo disso, adicione:
    
    if (aguardandoValor[msg.from]) {
        const etapa = aguardandoValor[msg.from].etapa;
        const resposta = texto.toLowerCase();
        const info = carregarInfo();
    
        if (info.valor100 == null) {
            delete aguardandoValor[msg.from];
            return msg.reply("⚠️ O valor de 100 Robux ainda não foi definido. Um administrador precisa usar /setvalor.");
        }
    
        // Etapa de escolha entre 1 ou 2
        if (etapa === "tipo") {
            if (resposta === "1") {
                aguardandoValor[msg.from] = { etapa: "robux" };
                return msg.reply("🔢 Digite a quantidade de *Robux* que você quer converter para reais.");
            } else if (resposta === "2") {
                aguardandoValor[msg.from] = { etapa: "reais" };
                return msg.reply("💵 Digite o valor em *Reais* que você quer converter para Robux.");
            } else {
                return msg.reply("❌ Resposta inválida. Digite apenas *1* para Robux ou *2* para Reais.");
            }
        }
    
        // Etapa de cálculo: Robux → Reais
        if (etapa === "robux") {
            const robux = parseInt(resposta);
            if (isNaN(robux) || robux <= 0 || robux > 100000) {
                delete aguardandoValor[msg.from];
                return msg.reply("❌ Quantidade de Robux inválida. Tente novamente com um número entre 1 e 100000.");
            }
    
            const valor = (robux / 100) * info.valor100;
            delete aguardandoValor[msg.from];
            return msg.reply(`💰 *${robux} Robux* custam aproximadamente *R$ ${valor.toFixed(2)}*`);
        }
    
        // Etapa de cálculo: Reais → Robux
        if (etapa === "reais") {
            const reais = parseFloat(resposta.replace(",", "."));
            if (isNaN(reais) || reais <= 0 || reais > 100000) {
                delete aguardandoValor[msg.from];
                return msg.reply("❌ Valor em reais inválido. Tente novamente com um número positivo.");
            }
    
            const robux = Math.floor((reais / info.valor100) * 100);
            delete aguardandoValor[msg.from];
            return msg.reply(`🎮 Com *R$ ${reais.toFixed(2)}* você pode comprar aproximadamente *${robux} Robux*`);
        }
    }
...
➡️ [WhatsApp +55 81 9513-2076](https://wa.me/558195132076)`;
    ...
    💰 Valor: ${valor ? `R$ ${valor}` : "Não especificado"}`;
            ...
            const partes = texto.split(" ");
            if (partes.length < 2) {
                return msg.reply("❌ Uso: /comprar QUANTIDADE_DE_ROBUX\nExemplo: /comprar 100");
            }
        
            const robux = parseInt(partes[1], 10);
            if (isNaN(robux) || robux <= 0 || robux > 100000) {
                return msg.reply("❌ Quantidade inválida! Informe um número inteiro positivo até 100000.");
            }
        
            const info = carregarInfo(); // ✅ valor carregado de info.json
        
            if (info.valor100 == null) {
                return msg.reply("⚠️ O valor de 100 Robux ainda não foi definido. Use */setvalor VALOR*.");
            }
        
            const valorReais = (robux / 100) * info.valor100;
            ...
            const nomeArquivo = `${chavePix}_${Date.now()}.png`;
            const comando = `${caminhoPython} "${caminhoScript}" "${chavePix}" ${valorReais.toFixed(2)} "${nomeArquivo}"`;
        
            msg.reply("💳 Gerando QR Code de pagamento, aguarde...");
        ...
        Você solicitou *${robux} Robux*.
        
        💵 *Valor a pagar:* R$ ${valorReais.toFixed(2)}
        🛒 *Game Pass (30% taxa):* *${valorGamepassRobux} Robux*
        
        ⚠️ Crie um Game Pass no Roblox com o valor de *${valorGamepassRobux} Robux* para receber corretamente.
        ...
        ➡️ [WhatsApp +55 81 9513-2076](https://wa.me/558195132076)`;
                        ...
                        await chat.sendMessage(media, { caption: textoCompra1 });
                        ...
                        console.error("Erro ao enviar ou ler imagem:", e);
                        chat.sendMessage("❌ Erro ao processar o QR Code.");
                    }
                }, 500);
            });
        }
        ...
        Para receber *${robux} Robux* líquidos,
        o valor do Game Pass deve ser de *${valorGamepassRobux} Robux*,
        considerando a taxa de 30% cobrada pelo Roblox.`;
            ...
            const partes = texto.split(" ");
            if (partes.length < 2) {
                return msg.reply("❌ Uso: /gift QUANTIDADE\nExemplo: /gift 143");
            }
            ...
            const valorReais = (robux / 143) * info.valor100;
        
            return msg.reply(`🎁 *GIFT Gamepass*\n\n🧮 ${robux} Robux com taxa equivalem a *R$ ${valorReais.toFixed(2)}*`);
        }
